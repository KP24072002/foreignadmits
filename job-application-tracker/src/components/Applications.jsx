...
const Applications = () => {
  ...
  const [editIndex, setEditIndex] = useState(null);  // Keep track of which application is being edited
  ...
  const handleEdit = (index) => {
    setEditIndex(index);  // Set the application index for editing
  };
  ...
  return (
    <div className="p-6">
      <AddApplicationForm 
        ...
        editIndex={editIndex}
        application={applications[editIndex]} 
      />
      <table className="w-full mt-6 bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg">
        ...
        <tbody>
          {applications.map((app, index) => (
            <tr
              key={index}
              className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-all"
            >
              ...
              <td className="p-4">
                ...
                <button 
                  className="text-red-500 hover:text-red-600 ml-2 transition-colors duration-300"
                  onClick={() => handleDelete(index)}  // Delete functionality
                >
